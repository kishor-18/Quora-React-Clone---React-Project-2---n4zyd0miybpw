...
const Login = ({ setUser }) => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(location.state?.isDarkMode || false);
  ...
  return (
    <div className={`login-page ${isDarkMode ? 'dark-mode' : 'light-mode'}`}>
      <div className="login-container">
        ...
        <p className="signup-link" onClick={() => navigate('/signup', { state: { isDarkMode } })}>Sign up with email</p>
      </div>
    </div>
  );
};
